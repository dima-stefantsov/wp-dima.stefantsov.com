...
class Momentous_Featured_Content {
	 ...
	 *
	 * @see Momentous_Featured_Content::init()
	 */
	public static $max_posts = 3;
	 ...
	 *
	 * @uses Momentous_Featured_Content::$max_posts
	 */
	 ...
	 * Get featured posts
	 *
	 * @uses Momentous_Featured_Content::get_featured_post_ids()
	 *
	 * @return array
	 */
	...
	public static function get_featured_post_ids() {
		...
		if ( ! empty( $featured_ids ) ) {
			return array_map( 'absint', apply_filters( 'momentous_featured_content_post_ids', (array) $featured_ids ) );
		}
		...
		return apply_filters( 'momentous_featured_content_post_ids', $featured_ids );
	}
	 ...
	 * @see Momentous_Featured_Content::validate_settings().
	 */
	 ...
	 *
	 * @uses Momentous_Featured_Content::get_featured_post_ids();
	 * @uses Momentous_Featured_Content::get_setting();
	 ...
	 * is deleted by Momentous_Featured_Content::validate_settings().
	 *
	 ...
	 *
	 * @uses Momentous_Featured_Content::validate_settings()
	 *
	 * @return void
	 */
	...
	public static function customize_register( $wp_customize ) {
		 ...
		 *
		 * Sanitization callback registered in Momentous_Featured_Content::validate_settings().
		...
		$wp_customize->add_control( 'featured-content[tag-name]', array(
			'label'          => esc_html__( 'Tag name', 'momentous-lite' ),
			'section'        => 'momentous_featured_content',
			'priority'       => 20,
		) );
		$wp_customize->add_control( 'featured-content[hide-tag]', array(
			'label'          => esc_html__( 'Hide tag from displaying in post meta and tag clouds.', 'momentous-lite' ),
			'section'        => 'momentous_featured_content',
			'type'           => 'checkbox',
			'priority'       => 30,
		) );
		$wp_customize->add_control( 'featured-content[show-all]', array(
			'label'          => esc_html__( 'Display featured posts in latest blog post listing.', 'momentous-lite' ),
			'section'        => 'momentous_featured_content',
			'type'           => 'checkbox',
			'priority'       => 40,
		) );
	...
	public static function enqueue_scripts() {
		wp_enqueue_script( 'momentous-featured-content-suggest', get_template_directory_uri() . '/js/featured-content-admin.js', array( 'jquery', 'suggest' ), '20150401', true );
	}
	 ...
	 * Momentous_Featured_Content::get_featured_content().
	 *
	 * @uses Momentous_Featured_Content::delete_transient()
	 *
	 * @param array $input
	 * @return array $output
	 */
